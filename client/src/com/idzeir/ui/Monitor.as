package com.idzeir.ui
{
	...
	public class Monitor extends Sprite 
	{
		...
		private function update(e:Event):void 
		{
			...
			var curMem:Number = System.privateMemory;
			maxMem = Math.max(maxMem, curMem);
			smoothFps = smoothFps * 0.99 + fps * 0.01;//控制平滑波动
			minFPS = Math.min(minFPS,smoothFps);
			infoTxt.text = "帧频:" + smoothFps.toFixed(2) + " 帧谷:" + minFPS + " 内存:" + Number(curMem * rate).toFixed(2) + " 峰值:" + Number(maxMem * rate).toFixed(2);
			prevTime = getTimer();
		}
